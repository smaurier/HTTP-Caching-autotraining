...
const hash3 = createHash("sha256").update("Hello!").digest("hex");
...
const etag = generateETag("Mon contenu");
const weakEtag = generateWeakETag("Mon contenu");
...
let serverContent = "Contenu initial de la ressource.";
let serverLastModified = new Date("2025-01-15T10:00:00Z");

const PORT = 3099;

const server = createServer((req, res) => {
  // Étape 4a : Calculer l'ETag du contenu actuel
  const etag = generateETag(serverContent);
  const lastModified = serverLastModified.toUTCString();
  ...
  if (clientEtag) {
    ...
    const etags = clientEtag.split(",").map((e) => e.trim());
    if (etags.includes(etag) || etags.includes("*")) {
      console.log(`  → 304 (ETag match: ${etag})`);
...
server.listen(PORT, async () => {
  ...
  const resB = await makeRequest("/data", {
    "If-None-Match": resA.headers.etag,
  });
  ...
  const oldEtag = resA.headers.etag;
  serverContent = "Contenu MODIFIÉ de la ressource !";
  serverLastModified = new Date();
  const resC = await makeRequest("/data", { "If-None-Match": oldEtag });
