...
function generateETag(content: string): string | undefined {
...
setTimeout(() => {
  ...
  // Vérifications
  const test1: boolean = etag1 === etag2;
  const test2: boolean = etag1 !== etag3;
  const test3: boolean = !!etag1?.startsWith('"') && !!etag1?.endsWith('"');
...
function generateWeakETag(content: string): string | undefined {
...
setTimeout(() => {
  ...
  const test1: boolean = !!weak?.startsWith('W/"');
  const test2: boolean = !!weak?.includes(strong?.replace(/"/g, "") ?? "");
