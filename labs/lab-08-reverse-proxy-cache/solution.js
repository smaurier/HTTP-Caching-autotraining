...
function getCacheKey(url, reqHeaders, varyHeader) {
  ...
  const varyParts = varyHeader.split(",").map((s) => s.trim().toLowerCase());
  const varyValues = varyParts
...
function getMaxAge(cacheControl) {
  ...
  if (cacheControl["max-age"]) {
    return parseInt(cacheControl["max-age"], 10) * 1000;
  }
  return 0;
}
...
const proxyServer = http.createServer((req, res) => {
  ...
  for (const [key, entry] of proxyCache) {
    ...
    if (entry.varyHeader) {
      const computedKey = getCacheKey(req.url, req.headers, entry.varyHeader);
  ...
  const proxyReq = http.request(
    ...
    (proxyRes) => {
      ...
      proxyRes.on("end", () => {
        const cc = parseCacheControl(proxyRes.headers["cache-control"]);
        const varyHeader = proxyRes.headers["vary"] || null;

        if (shouldCache(cc) && proxyRes.statusCode === 200) {
          const maxAge = getMaxAge(cc);
          if (maxAge > 0) {
            ...
            const cacheKey = getCacheKey(req.url, req.headers, varyHeader);
