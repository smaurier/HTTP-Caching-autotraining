 ...
 */
function handleResourceRequest(url, respond) {
...
const http1Server = createHttpServer((req, res) => {
  handleResourceRequest(req.url, (status, headers, body) => {
...
h2Server.on("stream", (stream, headers) => {
  ...
  handleResourceRequest(path, (status, headers, body) => {
 ...
 */
async function loadHttp1Sequential() {
  console.log("📦 Chargement SÉQUENTIEL via HTTP/1.1...\n");
  const start = performance.now();

  for (let i = 0; i < RESOURCE_COUNT; i++) {
    const reqStart = performance.now();
    const content = await http1Fetch(`/resource/${i}`);
 ...
 */
async function loadHttp2Multiplexed() {
...
http1Server.listen(HTTP1_PORT, () => {
  ...
  h2Server.listen(H2_PORT, async () => {
    ...
    try {
      const seqTime = await loadHttp1Sequential();
      const h2Time = await loadHttp2Multiplexed();
